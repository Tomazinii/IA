...
class Genetics_Algorithm_f6:
    ...
    def fitness(self,real):
        fitness_d = [ {"value":self.f6_function(element["x"], element["y"]),"value_x":element["x"],"value_y":element["y"]} for element in real ]
        fitness = [z["value"] for z in fitness_d]
        max_value = numpy.argmax(fitness)
        self.best_bit = max_value
        
        self.best_x = fitness_d[max_value]["value_x"]
        self.best_y = fitness_d[max_value]["value_y"]
    ...
    def start_generation(self):
        ...
        for ger in range(self.generation):
            ...
            if ger == self.generation - 1 :
                from matplotlib import pyplot as plt
                plt.plot(range(len(list_best_fitness)), list_best_fitness)
                plt.grid(True, zorder=0)
                plt.title("F6")
                plt.xlabel("geration")
                plt.ylabel("value f6 function")
                plt.show()

            print(f"Geração = {ger} || fitness={fitness[best_fitness]} || X={self.best_x} || Y={self.best_y} || bin={bit[self.best_bit]}  ")
